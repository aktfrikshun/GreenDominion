...
TMEdit.loadStyle("menu.css", "ContextMenu");
...
ContextMenu.prototype.onGenerate = function() {
	...
	TMEdit._addEvents(doc, ["contextmenu"],
			    function (event) {
				    return self.popupMenu(TMEdit.is_ie ? self.editor._iframe.contentWindow.event : event);
			    });
...
ContextMenu.prototype.popupMenu = function(ev) {
	...
	function documentClick(ev) {
		...
		var el = TMEdit.is_ie ? ev.srcElement : ev.target;
	...
	self.closeMenu = function() {
		...
		TMEdit._removeEvent(document, "mousedown", documentClick);
		TMEdit._removeEvent(self.editordoc, "mousedown", documentClick);
		if (keys.length > 0)
			TMEdit._removeEvent(self.editordoc, "keypress", keyPress);
		if (TMEdit.is_ie)
			self.iePopup.hide();
	};
	var target = TMEdit.is_ie ? ev.srcElement : ev.target;
	...
	if (!TMEdit.is_ie) {
		...
		doc = popup.document;
		doc.open();
		doc.write("<html><head><style type='text/css'>@import url(" + _editor_url + "plugins/ContextMenu/menu.css); html, body { padding: 0; margin: 0; overflow: hidden; border: 0px solid none; }</style></head><body unselectable='yes'></body></html>");
		doc.close();
	}
	...
	div.className = "tmedit-context-menu";
	if (!TMEdit.is_ie)
	...
	for (var i = 0; i < options.length; ++i) {
		...
		if (!option) {
			...
			if (TMEdit.is_ie) {
				td.unselectable = "on";
				IE_IS_A = " unselectable='on' style='height=1px'>&nbsp;";
			}
			td.innerHTML = "<div" + IE_IS_A + "</div>";
	...
	TMEdit._addEvent(document, "mousedown", documentClick);
	TMEdit._addEvent(this.editordoc, "mousedown", documentClick);
	if (keys.length > 0)
		TMEdit._addEvent(this.editordoc, "keypress", keyPress);
